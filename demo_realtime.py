...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        clips = list(Path("data/raw/nus48e/PMAR/sing").glob("*.wav")) + list(
            Path("data/raw/nus48e/PMAR/sing").glob("*.mp3")
        )
        if not clips:
    ...
    for i, clip in enumerate(clips):
        ...
        print(
            "  ⏳ Analyzing audio and generating timestamps... (takes a few seconds for full songs)"
        )
        ...
        if sr != 16000:
            ...
            audio = (
                torchaudio.transforms.Resample(sr, 16000)(torch.from_numpy(audio).unsqueeze(0))
                .squeeze(0)
                .numpy()
            )
