...
def get_transcription(audio_path, start, end):
    ...
    inp = proc.feature_extractor(audio, sampling_rate=16000, return_tensors="pt").input_features.to(
        device
    )
    with torch.no_grad():
...
for (singer, song), song_clips in grouped.items():
    ...
    for c in song_clips:
        ...
        for i in range(current_word_idx, search_window):
            for length in range(
                max(1, len(pred_words) - 5), min(len(lyrics_words) - i + 1, len(pred_words) + 8)
            ):
